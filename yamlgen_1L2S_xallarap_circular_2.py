...
os.mkdir("../" + sys.argv[1] + "/1L2S_xallarap_circular_2")
os.mkdir("../" + sys.argv[1] + "/1L2S_xallarap_circular_2/png")
...
for i in range(len(name)):
    if float(deltaChi[i]) > 10.0:
        xallarapPath.append('../sim_PAR/1L2S_xallarap_circular/' + name[i] + '.OUT')
...
for i, file in enumerate(xallarapName):
    ...
    yamlN = "../" + sys.argv[1] + "/1L2S_xallarap_circular_2/" + file + ".yaml"
    yamlN = open(yamlN, "w+")
    graphicF = sys.argv[1] + "/1L2S_xallarap_circular_2/png/" + file
    YAML = [
        "photometry_files:",
