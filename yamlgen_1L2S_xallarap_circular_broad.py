...
os.mkdir("../" + sys.argv[1] + "/1L2S_xallarap_circular_broad")
os.mkdir("../" + sys.argv[1] + "/1L2S_xallarap_circular_broad/png")
...
for i in range(len(name)):
    if float(deltaChi[i]) > 10.0:
        xallarapPath.append('../sim_PAR/xallarap_final_2/' + name[i] + '.OUT')
    ...
    with open(xallarapPath[i], "r") as fileOUT:
        ...
        for k in range(len(lines)):
            if "t_0" in lines[k] and "u_0" in lines[k]:
                ...
                u0_err = 10**(int(np.log10(abs(u0)))-1)
                break

    yamlN = "../" + sys.argv[1] + "/1L2S_xallarap_circular_broad/" + file + ".yaml"
    yamlN = open(yamlN, "w+")
    graphicF = sys.argv[1] + "/1L2S_xallarap_circular_broad/png/" + file
    YAML = [
        "photometry_files:",
