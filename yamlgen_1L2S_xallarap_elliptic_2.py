...
os.mkdir("../" + sys.argv[1] + "/1L2S_xallarap_elliptic_2")
os.mkdir("../" + sys.argv[1] + "/1L2S_xallarap_elliptic_2/png")
...
for i in range(len(name)):
    if float(deltaChi[i]) > 10.0:
        xallarapPath.append('../sim_PAR/1L2S_xallarap_elliptic/' + name[i] + '.OUT')
    ...
    with open(xallarapPath[i], "r") as fileOUT:
        ...
        for k in range(len(lines)):
            if "t_0" in lines[k] and "u_0" in lines[k]:
                ...
                xi_omega = float(tab["xi_omega_periapsis"])
                ...
                xi_e_err = 10**(int(np.log10(xi_e))-2)
                break

    yamlN = "../" + sys.argv[1] + "/1L2S_xallarap_elliptic_2/" + file + ".yaml"
    yamlN = open(yamlN, "w+")
    graphicF = sys.argv[1] + "/1L2S_xallarap_elliptic_2/png/" + file
    YAML = [
        "photometry_files:",
        ...
        "    xi_omega_periapsis: gauss " + str(xi_omega) + format(xi_e_err, '.10f'),
